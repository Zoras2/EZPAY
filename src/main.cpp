...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zEZPAY spend tx %s already in block %d",
            ...
            // Reject legacy zEZPAY mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zEZPAY mint output in tx %s",
...
int64_t GetBlockValue(int nHeight)
{
    int64_t nSubsidy = 0;
      ...
      if (nHeight == 0) {
        ...
        return 17.5* COIN;
	} else if (nHeight > 1000000 && nHeight <= 1200000) {
        return 15 * COIN;
	} else if (nHeight > 1200000 && nHeight <= 1400000) {
        return 12.5 * COIN;
	} else if (nHeight > 1400000 && nHeight <= 1600000) {
        return 10 * COIN;
	} else if (nHeight > 1600000 && nHeight <= 1800000) {
        return 7.5 * COIN;
	} else if (nHeight > 1800000 && nHeight <= 2000000) {
        return 5 * COIN;
	} else if (nHeight > 2000000 && nHeight <= 2270000) {
        return 2.5 * COIN;
	} else if (nHeight > 2270000) {
        return 2.5 * COIN;
	} else {
        return 2.5 * COIN;
      }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    //Track zEZPAY money supply
    if (!UpdateZPIVSupplyDisconnect(block, pindex))
        return error("%s: Failed to calculate new zEZPAY supply", __func__);
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // Update zEZPAY money supply map
    if (!UpdateZPIVSupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zEZPAY supply for block=%s height=%d", __func__,
                                    block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    }

    // A one-time event where the zEZPAY supply was off (due to serial duplication off-chain on main net)

...
/** Class encapsulating Ezpayza startup and shutdown.
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        // EZPAY: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", PIVXGUI::tr("A fatal error occurred. EZPAY can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    if (!boost::filesystem::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Ezpayza"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Ezpayza"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Ezpayza didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();

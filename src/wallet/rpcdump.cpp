...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Ezpayza address or script");
    }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"eazypayzaaddress\"   (string, required) The pivx address for the private key\n"
...
UniValue bip38encrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            "bip38encrypt \"eazypayzaaddress\" \"passphrase\"\n"
            "\nEncrypts a private key corresponding to 'eazypayzaaddress'.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"eazypayzaaddress\"   (string, required) The pivx address for the private key (you must hold the key already)\n"
...
UniValue bip38decrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            "bip38decrypt \"eazypayzaaddress\" \"passphrase\"\n"

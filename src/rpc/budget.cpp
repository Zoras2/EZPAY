                       ...
                       int &nPaymentCount, int &nBlockStart, CBitcoinAddress &address, CAmount &nAmount)
{
    ...
    if (nAmount < 10 * COIN)
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Invalid amount - Payment of %d is less than minimum 10 EZPAY allowed", FormatMoney(nAmount)));
...
UniValue preparebudget(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 6)
        throw std::runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"eazypayza-address\" monthy-payment\n"
            ...
            "5. \"eazypayza-address\":   (string, required) Ezpayza address to send payments to\n"
    ...
    CWalletTx wtx;
    if (!pwalletMain->GetBudgetSystemCollateralTX(wtx, budgetProposalBroadcast.GetHash(), useIX)) { // 50 EZPAY collateral for proposal
...
UniValue getbudgetprojection(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) Ezpayza address of payment\n"
